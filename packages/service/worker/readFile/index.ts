...
import { readImageFile } from './extension/image';
  ...
  async (
    ...
    const read = async (params: ReadRawTextByBuffer) => {
      switch (params.extension) {
        ...
        case 'png':
        case 'webp':
          return readImageFile(params);
        default:
          return Promise.reject(
            `Only support .txt, .md, .html, .pdf, .docx, pptx, .csv, .xlsx, .jpg, .jpeg, .png, .webp. "${params.extension}" is not supported.`
          );
      }
    };
