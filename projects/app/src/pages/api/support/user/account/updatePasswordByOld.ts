...
async function handler(
  ...
  // auth old password (avoid password condition setters)
  const user = await MongoUser.findById(userId).select('+password').lean();

  if (!user) {
    return Promise.reject(i18nT('common:user.Old password is error'));
  }

  const oldIsSha256 = /^[a-f0-9]{64}$/i.test(oldPsw);
  const old1 = oldIsSha256 ? oldPsw : hashStr(oldPsw);
  const old2 = hashStr(old1);
  if ((user as any).password !== old1 && (user as any).password !== old2) {
    return Promise.reject(i18nT('common:user.Old password is error'));
  }

  const newIsSha256 = /^[a-f0-9]{64}$/i.test(newPsw);
  const new1 = newIsSha256 ? newPsw : hashStr(newPsw);
  if (old1 === new1) {
