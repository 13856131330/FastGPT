...
import { processBufferWithPaddleOcr } from '@fastgpt/service/common/file/read/custom/paddleOcr';
  ...
  try {
    while (true) {
      ...
      try {
        ...
        let { title, rawText } = await (async () => {
          if (collection.metadata?.ocrProvider === 'paddleocr' && collection.fileId) {
            logger.info('Using PaddleOCR to process file', { fileId: collection.fileId });
            const s3DatasetSource = getS3DatasetSource();
            const { buffer, filename } = await s3DatasetSource.getDatasetFileBuffer({
              teamId: data.teamId,
              tmbId: data.tmbId,
              sourceId: String(collection.fileId)
            });

            const { formatText } = await processBufferWithPaddleOcr({
              buffer,
              filename
            });
            const text = await markdownProcess({ rawText: formatText });
            return { rawText: text, title: filename };
          }

          return readDatasetSourceRawText({
            teamId: data.teamId,
            tmbId: data.tmbId,
