...
export const readConfigData = async (name: string) => {
  ...
  const filename = (() => {
    ...
    const prodFile = `${envPath}/${name}`;
    if (existsSync(prodFile)) {
      return prodFile;
    }
    if (existsSync(`data/${devName}`)) {
      return `data/${devName}`;
    }
    if (existsSync(`data/${name}`)) {
      return `data/${name}`;
    }
    return prodFile;
  })();
...
export async function initSystemConfig() {
  ...
  // 初始化 OpenViking 集成
  if (config.openViking?.enabled) {
    const { initOpenVikingIntegration } = await import('@fastgpt/service/integration/openViking');
    await initOpenVikingIntegration(config.openViking);
  }
  ...
  logger.info('System config loaded', {
    fastgpt: {
      ...
      openVikingEnabled: config.openViking?.enabled || false
    }
  });
}
