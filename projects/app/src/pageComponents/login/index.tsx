...
const CookiesModal = () => {
  ...
  const [, setLocalCookieVersion] = useLocalStorageState<string>('localCookieVersion');

  useEffect(() => {
    // 默认同意 Cookie 政策：写入版本标记并不弹窗
    setLocalCookieVersion(cookieVersion);
  }, [cookieVersion, setLocalCookieVersion]);
