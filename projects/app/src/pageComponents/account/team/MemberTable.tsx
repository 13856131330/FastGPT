...
import { delAdminDeleteUser, putAdminUpdateUserStatus } from '@/web/support/user/admin/api';
...
const CreateUserModal = dynamic(() => import('@/pageComponents/admin/CreateUserModal'));
...
function MemberTable({ Tabs }: { Tabs: React.ReactNode }) {
  ...
  const isEmployeePage = router.pathname.startsWith('/admin/employee');
  ...
  const [status, setStatus] = useState<string>(() => (isEmployeePage ? 'all' : 'active'));
  ...
  const { runAsync: onDisableUser, loading: disablingUser } = useRequest(
    (userId: string) => putAdminUpdateUserStatus({ userId, status: UserStatusEnum.forbidden }),
    {
      manual: true,
      onSuccess: onRefreshMembers,
      successToast: '已停用'
  ...
  const { runAsync: onEnableUser, loading: enablingUser } = useRequest(
    (userId: string) => putAdminUpdateUserStatus({ userId, status: UserStatusEnum.active }),
    {
      manual: true,
      onSuccess: onRefreshMembers,
      successToast: '已启用'
  ...
  const { runAsync: onDeleteUser, loading: deletingUser } = useRequest(
    (userId: string) => delAdminDeleteUser({ userId }),
    {
      manual: true,
      onSuccess: onRefreshMembers,
      successToast: '已删除'
  ...
  const isLoading = loadingMembers || isSyncing || disablingUser || enablingUser || deletingUser;
    ...
    <>
      <Flex justify={'space-between'} align={'center'} pb={'1rem'}>
        ...
        <HStack alignItems={'center'}>
          ...
          {isAdminPage && userInfo?.username === 'root' && !isSyncMode && !isWecomTeam && (
            <Button
              variant={'whitePrimary'}
              size="md"
              borderRadius={'md'}
                  ...
                  <Tr key={member.tmbId} overflow={'unset'}>
                    <Td>
                      <HStack>
                        ...
                        <Box className={'textEllipsis'}>
                          {member.memberName}
                          {(member as any)?.userStatus === 'forbidden' && (
                    ...
                    <Td>
                      {userInfo?.team.permission.hasManagePer &&
                        ...
                        (!isEmployeePage && member.status === TeamMemberStatusEnum.active ? (
                          <HStack>
                            <MyIconButton
                              icon={'edit'}
                              size="1rem"
                          ...
                          // 员工管理：停用（账号级）+ 删除（账号级）
                          <HStack>
                            {(member as any)?.userStatus === UserStatusEnum.forbidden ? (
                              <PopoverConfirm
                                Trigger={
                                  <Box>
                                    <MyIconButton
                                ...
                                type="info"
                                content={`启用账号：${(member as any)?.username || member.memberName}？`}
                                onConfirm={() => onEnableUser((member as any).userId)}
                              />
                            ) : (
                              <PopoverConfirm
                                Trigger={
                                ...
                                type="info"
                                content={`停用账号：${(member as any)?.username || member.memberName}？`}
                                onConfirm={() => onDisableUser((member as any).userId)}
                              />
                            )}
                            ...
                            <PopoverConfirm
                              ...
                              type="delete"
                              content={`删除账号：${(member as any)?.username || member.memberName}（不可恢复）？`}
                              onConfirm={() => onDeleteUser((member as any).userId)}
                            />
                          </HStack>
                        ))}
                    </Td>
