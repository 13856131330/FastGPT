...
const CollectionCard = () => {
  ...
  const { runAsync: onclickStartSync, loading: isSyncing } = useRequest(
    async () => {
      const result = await postOpenVikingDatasetSync({
        datasetId: datasetDetail._id,
        mode: 'full'
      });
      if (result?.status === 'failed') {
        throw new Error(result.error || 'OpenViking sync failed');
      }
      return result;
    },
    {
      ...
      errorToast: t('dataset:sync_collection_failed')
    }
  );
