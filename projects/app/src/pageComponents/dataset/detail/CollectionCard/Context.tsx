...
import { getDatasetCollections, postOpenVikingDatasetSync } from '@/web/core/dataset/api';
...
const CollectionPageContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const syncDataset = useCallback(async () => {
    ...
    const result = await postOpenVikingDatasetSync({ datasetId: datasetId, mode: 'full' });
