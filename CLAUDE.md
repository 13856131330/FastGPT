...
- `pnpm test:workflow` - 专门运行工作流相关测试
- `pnpm lint` - 对所有 TypeScript 文件运行 ESLint 并自动修复
- `pnpm format-code` - 使用 Prettier 格式化代码

### 运行单个测试
- `pnpm test -- --run <test-file-path>` - 运行特定测试文件
- `pnpm test -- --run <test-file-path> -t "test name"` - 运行特定测试用例
- 测试文件位于 `test/` 目录和 `projects/app/test/` 中
...
- `pnpm test:vector` - 运行向量数据库相关测试
- 测试文件位于 `test/` 目录和 `projects/app/test/`
- 覆盖率报告生成在 `coverage/` 目录

## 环境配置

- 主配置文件在 `projects/app/data/config.json`
- 本地开发配置可覆盖 `projects/app/data/config.local.json` (使用 JSON5 格式，支持注释)
- 环境变量通过 `packages/service/env.ts` 和 `projects/app/src/service/env.ts` 管理
- 数据库连接、AI 提供商密钥等敏感信息应通过环境变量配置
...
### 工作流引擎
- 工作流引擎核心在 `packages/service/core/workflow/`
- 节点定义在 `packages/service/core/workflow/nodes/`
- 工作流执行逻辑在 `packages/service/core/workflow/run/`

## 开发注意事项

- **包管理器**: 使用 pnpm 及 workspace 配置
- **Node 版本**: 需要 Node.js >=20，pnpm >=10.x (参见 package.json engines)
- **数据库**: 支持 MongoDB、带 pgvector 的 PostgreSQL 或 Milvus 向量存储
- **AI 集成**: 通过统一接口支持多个 AI 提供商，配置在 `packages/service/core/ai/config/`
- **国际化**: 完整支持中文、英文和日文，使用 i18next 和 next-i18next

## 关键文件模式

- `.ts` 和 `.tsx` 文件全部使用 TypeScript
- 数据库模型使用 Mongoose 配合 TypeScript
- API 路由遵循 NextJS 约定，使用 Zod 进行输入验证
...
### 添加新 API 路由
1. 在 `projects/app/src/pages/api/` 创建新的路由文件
2. 定义 Zod schema 进行输入验证
3. 在 `packages/global/openapi/` 中添加 OpenAPI 文档
4. 业务逻辑放在 `packages/service/` 或 `projects/app/src/service/`
5. 遵循 `api-development` skill 规范

### 添加新前端组件
1. 如果是共享组件，放在 `packages/web/components/`
2. 如果是应用特定组件，放在 `projects/app/src/components/`
3. 使用 Chakra UI 进行样式设计
4. 支持国际化，使用 `i18nT` 函数

### 添加新工作流节点
1. 在 `packages/service/core/workflow/nodes/` 创建节点定义
2. 实现节点逻辑和输入输出定义
3. 在 `packages/service/core/workflow/constants.ts` 中注册节点
4. 添加相应的测试

## Agent 设计规范

1. 对于功能的实现和复杂问题修复，优先进行文档设计，并与用户确认后，再进行执行修复。
2. 采用"设计文档-测试示例-代码编写-测试运行-修正代码/文档"的工作模式，以测试为核心来确保设计的正确性。
3. 设计文档保存在 `.claude/design/`，问题分析文档保存在 `.claude/issue/`。
4. 相同需求文档尽量写在一起，避免平铺不同版本的相同问题文档。